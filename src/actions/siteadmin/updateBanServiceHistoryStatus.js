import { gql } from 'react-apollo';
import { sendEmail } from '../../core/email/sendEmail';
// Toaster
import { toastr } from 'react-redux-toastr';
import { adminEmail } from '../../config';
const adminMail = adminEmail.email;
const query = gql`
query{
    userManagement
    {
        id,
        email,
        profile
        {
        profileId,
        firstName,
        lastName,
        gender,
        dateOfBirth,
        phoneNumber,
        preferredLanguage,
        preferredCurrency,
        location,
        info,
        createdAt
        },
        userBanStatus
    }
}
`;
const mutation = gql`
  mutation($id: String, $banStatus: Int) {
    updateBanServiceHistoryStatus(id: $id ,banStatus: $banStatus){
      status
      }
    }
  `;
export function updateBanServiceHistoryStatus(id, banStatus, userMail, userName) {
    return async (dispatch, getState, { client }) => {
        try {
            const { data } = await client.mutate({
                mutation,
                variables: { id, banStatus },
                fetchPolicy: 'network-only',
                refetchQueries: [{ query }]
            });
            if ((data.updateBanServiceHistoryStatus.status === "success")) {
                let mailData = {
                    userName,
                    userMail,
                    adminMail
                }
                toastr.success('Update Service Status', "Service Status updated successfully");
                if (banStatus == '1') {
                    await sendEmail(userMail, 'banStatusServiceStatusBanned', mailData);
                } else if (banStatus == '0') {
                    await sendEmail(userMail, 'banStatusServiceStatusUnBanned', mailData);
                }
            } else {
                toastr.error('Update Service Status', "Service Status updation failed");
            }
        } catch (error) {
            toastr.warning('Select Ban/Unban', "You have to select either one option ban/unban");
            return false;
        }
        return true;
    };
}
